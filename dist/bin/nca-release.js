...
function run() {
    ...
    if (NPM_EMAIL && NPM_AUTH) {
        var npmPublishResult = executor.publishNpm(NPM_EMAIL, NPM_AUTH);
        if (npmPublishResult) {
            console.log(_chalk2.default.green('npm publish "' + checker.logVersion + '" succeeded.'));
        } else {
            console.log(_chalk2.default.red('npm publish "' + checker.logVersion + '" failed.'));
        }
    } else {
